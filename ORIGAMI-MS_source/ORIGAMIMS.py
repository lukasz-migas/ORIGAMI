...
class ORIGAMIMS(object):
    ...
    def init(self, *args, **kwargs):
        ...
        self.view = mainWindow.MyFrame(self, config=self.config, title="ORIGAMI-MS")
        self.wrensCMD = None
        self.wrensRun = None
...
#===============================================================================
    ...
    def onCheckParameters(self, evt):
        ...
        if (abs(self.config.iStartVoltage) < 0):
            print('The lowest possible voltage is 0 V')
        ...
        if self.config.iSPV <= 0:
            print('SPV must be larger than 0! Set to default: 3')
            self.config.iSPV = 3
            self.view.panelControls.spv_input.SetValue(str(self.config.iSPV))
        
        if self.config.iScanTime <= 0:
            print('Scan time must be larger than 0! Set to default: 5')
        ...
        if self.config.iActivationMode == "Exponential":
            if self.config.iExponentPerct <= 0:
                print('Exponential % must be larger than 0! Set to default: 0')
                self.config.iExponentPerct = 0
            elif self.config.iExponentPerct >= 100:
                print('Exponential % must be smaller than 100! Set to default: 0')
                self.config.iExponentPerct = 0
            self.view.panelControls.exponentialPerct_input.SetValue(str(self.config.iExponentPerct))
            
            if self.config.iExponentIncre <= 0:
                print('Exponential increment must be larger than 0! Set to default: 0')
                self.config.iExponentIncre = 0
            elif self.config.iExponentIncre >= 0.05:
                print('Exponential increment must be smaller than 0.05! Set to default: 0')
                self.config.iExponentIncre = 0
            self.view.panelControls.exponentialIncrm_input.SetValue(str(self.config.iExponentIncre))
        elif self.config.iActivationMode == "Boltzmann":
            if self.config.iBoltzmann <= 10:
                print('Boltzmann offset must be larger than 10! Set to default: 0')
                self.config.iBoltzmann = 0
            elif self.config.iBoltzmann >= 100:
                print('Boltzmann offset must be smaller than 100! Set to default: 0')
                self.config.iBoltzmann = 0
    ...
    def onCalculateParameters(self, evt):
        ...
        if not self.config.iActivationMode == "User-defined":
            ...
            if divisibleCheck2 != 0:
                print("Are you sure your collision voltage range is divisible by your increment?")
                return
        else:
            if self.config.iScanTime == None or self.config.iScanTime == "": 
                print('Please make sure you to fill in the scan time input box.')
                return
        ...
        self.view.SetStatusText(''.join(['# steps: ',str(len(scanPerVoltageList))]), number=1)
    ...
    def onPrepareLinearMethod(self):
        ...
        if (totalAcqTime > 300):
            print("The acquisition will take more than 5 hours. Consider reducing " \
        ...
        wrensInput = {'polarity':self.config.iPolarity,
                     ...
                     'command':wrensCMD}
        self.view.SetStatusText(''.join(["Current method in ",self.config.iPolarity,
                                          " in the ",self.config.iActivationZone,
                                          " using the ",self.config.iActivationMode]), number=2)
    ...
    def onPrepareListMethod(self):
        ...
        print('im in')
        if self.config.CSVFilePath == None: return       
        if self.config.iScanTime == None or self.config.iScanTime == "": return
...
#         timeList = scanPerVoltageList*self.config.iScanTime
    ...
    def onStartWREnSRun(self, evt):
        ...
        if self.wrensCMD == None: 
            ...
            dialogs.dlgBox(exceptionTitle='Please complete all necessary fields', 
                           exceptionMsg= msg,
                           type="Error")
            return
        ...
        if self.wrensInput.get("polarity",None) != self.config.iPolarity: 
            print('The polarity of the current method and the one in the window do not agree. Consider re-calculating.')
            return
        if self.wrensInput.get("activationZone",None) != self.config.iActivationZone: 
            print('The activation zone of the current method and the one in the window do not agree. Consider re-calculating.')
            return
        if self.wrensInput.get("method",None) != self.config.iActivationMode: 
            print('The acquisition mode of the current method and the one in the window do not agree. Consider re-calculating.')
            return
        if self.wrensInput.get("command",None) != self.wrensCMD: 
            print('The command in the memory and the current method and the one in the window do not agree. Consider re-calculating.')
            return
        

        print("Your code: ",self.wrensCMD)
...
#             self.view.SetStatusText(dlg.GetPath(), number=2)#
