...
autopep8
autopep8 --in-place --aggressive --aggressive --max-line-length=120 ORIGAMI.py
...
GUI:
- [changed] renamed the Menu item "Plugins" to "Widgets"

Document object:
- [new] added new keyword "metadata" which will store parameters for verious processes

Main panel:
- [changed] Removed 'Process' from the Menu
- [changed] Removed 'Processing' buttons from the toolbar - now, you have to right-click on an item to process it
    (should remove a lot of confusion of what is being processed)
- [new] added export configuration file to the toolbar
...
- [improved] reorganised the menu a little so its cleaner and easier to navigate
- [new] added option to 'Process all...' items in grouped items (e.g. Mass Spectra, Drift time (2D, EIC), etc). Selecting
    this option will open new window where you can specify your desired parameters  and click on 'Add to document' which
    will process each item and add it to the document.
TODO: Add option to change x/y-axis for ALL items in specified branch
TODO: Add option to change charge for AL items in specified branch
- [new] you can assign charge state for single or multiple items in a sub-tree element.
...
- [changed] comparing mass spectra is a little bit better now. I've modified the Comparison widget to have its own
    plotting window so the behaviour is a lot more understandable! Now when you double-click on Mass Spectra in the Document
    panel or select Menu -> Widgets -> Compare spectra... a new window will appear where you can easily compare
    spectra from within the same or different document. Plotting is automatic so you simply select the spectrum you want
    to view.
- [changed] I've cleaned-up the main plotting panel by renaming various compoents (e.g. MS -> Mass spectrum, RT ->
    Chromatogram, etc)
- [improved] I've removed various plot windows so its easier to understand where something has been plotted. Removed
    windows include: UniDec (now has own widget), Overlay (now shown in Heatmap), RMSF (shown in Heatmap), Comparison
    (shown in Heatmap)..
- [new] Added option to copy plot to clipboard. Simply right-click in any of the plots and select 'Copy plot to clipboard'
...
- [new] added new peak picking module that can be activated when you right-click in the document view on any
    MS dataset and select `Open peak picker...` which will open a new window where youc an select between two different
    peak picking algorithms:
        - one that is pretty good for small molecules which is based on the principle of peak properties
        - one that is pretty good for native MS and is based on local maxima search - data should be smoothed first
- [changed] moved the processing MS module away from the multi-page window into its own, individual frame which can be
    accessed by right-clicking on a MS item in the Document Tree and selecting 'Process...'
- [new] added new normalization methods in the MS module
- [new] added new smoothing method (moving average) in the MS module
- [new] added new baseline subtraction methods (polynomial and curved) in the MS module
- [changed] moved the processing 2D module away from the multi-page window into its own, individual frame which can be
    accessed by right-clicking on a 2D/heatmap item in the Document tree and selecting 'Process...'
- [new] added cropping to the 2D module
- [new] added interpolation to the 2D module
- [changed] moved the processing `Extract data` module away from the multi-page window into its own, individual frame
    which can be accessed by right-clicking anywhere on the document and selecting
    `Action -> Open data extraction panel...`
- [improved] Extracting arbirary data from Waters files (using Driftscope) is a lot improved as I've removed whacky
    controls that were causing all sorts of UI issues. Data extraction is also done in a multi-threaded manner, which
    should speed things up
- [changed] Moved UniDec deconvolution to its own, separate panel which will allow for easier to navigate experience.
    The panel has its plotting window, meaning you can simply process and view data in one place rather than having
    two separate windows.
    Visualizing UniDec data is also easy. When you double-click (or right-click) on an item in the Document Tree,
    the same panel will open, with UniDec results
- [improved] Changed UniDec version to the latest (v3.2.0)
- [fixed] Visualising charge values on the `Isolated MS' will now show vertical lines at correct values, corresponding
    to the adduct selection
- [new] added a couple of more adduct types to the Adduct selection
- [improved] various controls in the UniDec panel will not automatically plot/replot data when an event is triggered
- [improved] error messages will be clearer from now on
