...
class OrigamiConfig:
    ...
    def __init__(self):
        ...
        self.overlayChoices = sorted([
            'Mask', 'Transparent', 'RGB', 'Mean',
            'Variance', 'Standard Deviation',
            'RMSD', 'RMSF', 'RMSD Matrix',
            'Grid (2->1)', 'Grid (n x n)',
        ])

        self.comboAcqTypeSelectChoices = [
            'Linear', 'Exponential', 'Fitted',
            'User-defined',
        ]
        ...
        self.availablePlotsList = [
            'MS', 'MS (compare)', 'RT', 'DT', '2D', '3D', 'DT/MS', 'Waterfall',
            'RMSD', 'Comparison', 'Overlay', 'Calibration (MS)',
            'Calibration (DT)',
        ]

        self.markerShapeDict = {
            'square': 's', 'circle': 'o', 'pentagon': 'p', 'star': '*',
            'diamond': 'D', 'cross': 'x',
        }
        ...
        self.rmsdLocChoices = {
            'leftTop': (5, 95), 'rightTop': (75, 95), 'leftBottom': (5, 5),
            'rightBottom': (75, 5), 'None': (),
        }

        self.interactiveToolsOnOff = {
            '1D': {
                'hover': True, 'save': True, 'pan': True,
        ...
        self._plotSettings = {
            'MS': {
                'axes_size': [0.1, 0.5, 0.85, 0.45], 'gui_size': [10, 6], 'resize_size': [10, 4],
                'save_size': [0.1, 0.15, 0.85, 0.8], 'default_name': 'MS',
            },
            'MS (compare)': {
                'axes_size': [0.1, 0.1, 0.8, 0.85], 'gui_size': [8, 8],
                'resize_size': [10, 10], 'save_size': [0.1, 0.1, 0.8, 0.85], 'default_name': 'MS_compare',
            },
            'RT': {
                'axes_size': [0.1, 0.2, 0.8, 0.75], 'gui_size': [10, 4],
            ...
            '3D': {
                'axes_size': [0.1, 0.1, 0.8, 0.85], 'gui_size': [12, 8],
            ...
            'Comparison': {
                'axes_size': [0.2, 0.2, 0.6, 0.6], 'gui_size': [8, 8],
            ...
            'UniDec (m/z vs Charge)': {
                'axes_size': [0.1, 0.1, 0.8, 0.85], 'gui_size': [6, 6],
            ...
            'UniDec (Barplot)': {
                'axes_size': [0.15, 0.1, 0.8, 0.85], 'gui_size': [6, 6],
                'resize_size': [10, 6], 'save_size': [0.1, 0.1, 0.8, 0.85],
                'default_name': 'unidec_barplot',
            },
            'UniDec (Charge Distribution)': {
                'axes_size': [0.15, 0.1, 0.7, 0.75], 'gui_size': [6, 3],
                'resize_size': [10, 6], 'save_size': [0.1, 0.1, 0.8, 0.85],
                'default_name': 'unidec_chargeDist',
            },
            'Other (Line)': {
                'axes_size': [0.15, 0.45, 0.75, 0.5], 'gui_size': [10, 6],
                'resize_size': [10, 4], 'save_size': [0.1, 0.1, 0.8, 0.85], 'default_name': 'other_line',
            },
            'Other (Multi-line)': {
                'axes_size': [0.15, 0.2, 0.7, 0.7], 'gui_size': [6, 3],
                'resize_size': [10, 6], 'save_size': [0.1, 0.1, 0.8, 0.85],
        ...
        self.driftTopColNames = {
            'start': 0, 'end': 1, 'scans': 2, 'drift_voltage': 4,
            'filename': 5,
        }

        self.driftBottomColNames = {
            'start': 0, 'end': 1, 'intensity': 2, 'charge': 4,
            'filename': 5,
        }

        self.textlistColNames = {
            'check': 0, 'start': 1, 'end': 2, 'charge': 3, 'color': 4,
        ...
        self.interactiveColNames = {
            'check': 0, 'document': 1, 'type': 2, 'file': 3, 'title': 4,
            'header': 5, 'footnote': 6, 'color': 7, 'colormap': 7,
            'page': 8, 'order': 9,
        }

        self.ccsTopColNames = {
            'filename': 0, 'start': 1, 'end': 2,
            'protein': 3, 'charge': 4, 'ccs': 5, 'tD': 6,
            'gas': 7,
        }

        self.ccsBottomColNames = {
            'filename': 0, 'start': 1, 'end': 2,
        ...
        self.ccsDBColNames = {
            'protein': 0, 'mw': 1, 'units': 2, 'charge': 3, 'ion': 4,
        ...
        self.interactive_activeDragTools_choices = \
            ['Box Zoom', 'Box Zoom X', 'Box Zoom Y', 'Pan', 'Pan X', 'Pan Y', 'auto', 'None']
        ...
        self.customColors = OrderedDict([
            (0, [16, 71, 185]), (1, [50, 140, 0]), (2, [241, 144, 0]),
        ...
        self._windowSettings = {
            ...
            'Toolbar': {
                'gripper': True, 'show': True, 'orientation': 'top',
        ...
        self.normalization2D_choices = \
            ['Maximum', 'Logarithmic', 'Natural log', 'Square root', 'Least Abs Deviation', 'Least Squares']
        ...
        self.smart_zoom_downsampling_method_choices = [
            'Auto', 'Binned (summed)', 'Binned (mean)', 'Sub-sampled',
        ]  # new in v1.3.0.0
        ...
        self.peak_find_min_intensity = 0.  # new in v1.3.0.0
...
# ===============================================================================
    ...
    def get_waters_inf_data(self, path):
        '''
        Imports information file for selected MassLynx file
        '''
        fileName = r'\_extern.inf'
        fileName = ''.join([path, fileName])

        parameters = OrderedDict.fromkeys(
            [
                'startMS', 'endMS', 'setMS', 'scanTime', 'ionPolarity',
                'modeSensitivity', 'pusherFreq', 'corrC', 'trapCE',
            ], None,
        )
    ...
    def get_origami_config_data(self, path):
        ...
        try:
            ...
            self.origamiList = np.genfromtxt(
                spvPath, skip_header=1,
                delimiter=',',
                filling_values=0,
            )
        except Exception:
    ...
    def loadConfigXML(self, path, evt=None):
        ...
        tagList = [
            'process_presets_overlay', 'process_presets_binning',
            ...
            if pathTags:
                ...
                for pathTag in pathTags:
                    self.previousFiles.append({
                        'file_path': pathTag.getAttribute('value'),
...
#                                             'default_name':default_name}
        ...
        if interactiveTags:
            ...
            for item in xmlTags:
                ...
                page_dict = {}
                for key_item in [
                    'name', 'layout', 'rows', 'columns', 'grid_share_tools',
                ]:
                    value = item.getAttribute(key_item)
                    # Get value of proper type
                    value = self.setProperType(value, 'str')
                ...
                for item in xmlTags:
                    ...
                    if title != 'Toolbar':
                        ...
                        self._windowSettings[title] = {
                            'title': title, 'gripper': gripper, 'caption': caption,
