...
#             self.data_handling.on_save_document_fcn(document_title)
    ...
    def onLoadOtherData(self, fname):
        ...
        msg = "Item {} has: x-columns ({}), x-errors ({}), y-columns ({}), x-errors ({}), ".format(os.path.basename(fname), len(
            xvals), len(xvalsErr), len(yvals), len(yvalsErr)) + "labels ({}), colors ({})".format(len(labels), len(colors))
        print(msg)
...
#         self.Bind(wx.EVT_MENU, self.on_save_all_documents, id=ID_saveAllDocuments)
        ...
        if ((self._document_type in ['Chromatogram'] and self._indent == 2) or (self._document_type in [
                'Chromatograms (EIC)', 'Chromatograms (combined voltages, EIC)'] and self._indent > 2)):
            try:
            ...
            if (self._document_type in ['Drift time (2D)', 'Drift time (2D, processed)']
                ...
                if self._item_leaf == "Annotations":
                    ...
                    menu.AppendItem(makeMenuItem(parent=menu,
                                                 ...
                                                 text='Save results - {} ({})'.format(self._item_leaf,
        ...
        # Drift time (2D)
            ...
            if (
                self._document_type in [
                    'Drift time (2D)', 'Drift time (2D, processed)'] or (
                    self._document_type == 'Drift time (2D, EIC)' and self._item_leaf != self._document_type) or (
                    self._document_type == 'Drift time (2D, combined voltages, EIC)' and self._item_leaf != self._document_type) or (
    ...
    def onRenameItem(self, evt):
        ...
        if new_name == current_name:
            ...
            if self._indent == 1:
                ...
                try:
                    self.presenter.view.panelMML.onRenameItem(current_name,
                                                              new_name,
                                                              item_type="document")
                except Exception:
                    pass
                try:
                    self.presenter.view.panelMultipleIons.onRenameItem(current_name,
                                                                       new_name,
                                                                       item_type="document")
                except Exception:
                    pass
    ...
    def onShowUnidec(self, evt, plot_type="all"):
        ...
        if plot_type in ["all", "Fitted", "Processed"]:
            try:
                ...
                try:
                    self.panel_plot.on_plot_unidec_MS(replot=unidec_engine_data['Processed'],
                                                      **kwargs)
                except Exception:
        ...
        if plot_type in ["all", "MW vs Charge"]:
            try:
                self.panel_plot.on_plot_unidec_MW_v_Charge(replot=unidec_engine_data['MW vs Charge'],
                                                           **kwargs)
            except Exception:
        ...
        if plot_type in ["all", "Barchart"]:
            try:
                self.panel_plot.on_plot_unidec_barChart(replot=unidec_engine_data['Barchart'],
                                                        **kwargs)
            except Exception:
    ...
    def onSaveDF(self, evt):
        ...
        if self._document_type == 'Mass Spectra' and self._item_leaf == 'Mass Spectra':
            ...
            try:
                ...
                print(('Dataframe was saved in %s. It took: %s s.' %
        ...
        if (self._document_type == 'Mass Spectrum' or self._document_type == 'Mass Spectrum (processed)' or
            ...
            if self._document_type == 'Mass Spectrum':
                ...
                defaultValue = "MS_{}_{}{}".format(
                    basename,
                    extractBasename,
                    self.config.saveExtension).replace(
                    ":",
                    "")
            ...
            for annotData in docData.multipleMassSpectrum:
                ...
                if 'unidec' in docData.multipleMassSpectrum[annotData]:
                    ...
                    for annotDataInner in docData.multipleMassSpectrum[annotData]['unidec']:
                        ...
                        self.SetPyData(annotsItemInner,
                ...
                if ('annotations' in docData.multipleMassSpectrum[annotData] and
                    ...
                    for annotNameData in docData.multipleMassSpectrum[annotData]['annotations']:
                        ...
                        self.SetPyData(annotsAnnotItem,
        ...
        if selected is not None:
            ...
            try:
                ...
                if text != 'Documents':
                    ...
                    #                     except Exception: pass
                    self.presenter.currentDoc = text
                    self.view.SetTitle("ORIGAMI - %s - %s (%s)" %
