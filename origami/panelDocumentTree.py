...
class documentsTree(wx.TreeCtrl):
    ...
    def __init__(
        self, parent, mainParent, presenter, icons, config, id, size=(-1, -1),
...
#                 self.removeDocument(evt=ID_removeDocument)
    ...
    def on_double_click(self, evt):
        ...
        if itemType == 'Documents':
            ...
            menu.AppendItem(
                makeMenuItem(
                    parent=menu, id=ID_saveAllDocuments,
            ...
            menu.AppendItem(
                makeMenuItem(
                    parent=menu, id=ID_removeAllDocuments,
        ...
        self._indent = self.getItemIndent(item)
        if self._indent > 1:
        ...
        if self._indent == 1 and self._item_leaf is None:
            ...
            if (
                ...
                self.onShowUnidec(None, plot_type=self._item_leaf)
    ...
    def _resetBullets(self):
        ...
        self.bulets_dict = {
            'document': 0, 'mass_spec': 1, 'drift_time': 2,
...
#             self.data_handling.on_save_document_fcn(document_title)
    ...
    def onLoadInteractiveData(self, evt):
        ...
        dlg = wx.FileDialog(
            self.presenter.view, 'Choose data [MS, RT, DT]...',
            wildcard=wildcard,
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for path, fname in zip(pathlist, filenames):
                if evtID == ID_docTree_add_MS_to_interactive:
                    ...
                    data = {
                        'zvals': imsData2D, 'xvals': xAxisLabels,
    ...
    def onLoadOtherData(self, fname):
        ...
        for xy_axis in [
            'axis_x', 'axis_y', 'axis_xerr', 'axis_yerr', 'axis_color', 'axis_colors',
        ...
        msg = 'Item {} has: x-columns ({}), x-errors ({}), y-columns ({}), x-errors ({}), '.format(
            os.path.basename(fname), len(
                xvals,
            ), len(xvalsErr), len(yvals), len(yvalsErr),
        ) + 'labels ({}), colors ({})'.format(len(labels), len(colors))
        print(msg)

        # update title
        _plot_types = {
            'multi-line': 'Multi-line', 'scatter': 'Scatter',
    ...
    def onAddAnnotation(self, evt):
        ...
        # Annotated data
            ...
            'Multi-line: ' in self._item_leaf or 'Multi-line: ' in self._item_branch or
            'V-bar: ' in self._item_leaf or 'V-bar: ' in self._item_branch or
            'H-bar: ' in self._item_leaf or 'H-bar: ' in self._item_branch or
            'Scatter: ' in self._item_leaf or 'Scatter: ' in self._item_branch or
            'Waterfall: ' in self._item_leaf or 'Waterfall: ' in self._item_branch or
            'Line: ' in self._item_leaf or 'Line: ' in self._item_branch
        ):
            data = None
    ...
    def on_get_annotation_dataset(self, document, dataset):
        ...
        if dataset == 'Mass Spectrum':
            ...
            'Multi-line: ' in dataset or 'V-bar: ' in dataset or
    ...
    def onDuplicateAnnotations(self, evt):
        ...
        if annotations is None or len(annotations) == 0:
            self.presenter.onThreading(
                None, ('This item has no annotations to duplicate.', 4, 5),
                action='updateStatusbar',
            )
            return
        ...
        if duplicate_document is None or duplicate_dataset is None:
            self.presenter.onThreading(
                None, ('Operation was cancelled.', 4, 5),
                action='updateStatusbar',
            )
            return
    ...
    def onShowAnnotations(self, evt):
        ...
        if self._document_type == 'Mass Spectrum':
            ...
            self._document_type == 'Annotated data' and
            (
                'Waterfall: ' in self._item_branch or 'Multi-line: ' in self._item_branch or
                'V-bar: ' in self._item_branch or 'H-bar: ' in self._item_branch or
                'Scatter: ' in self._item_branch or 'Line: ' in self._item_branch
            )
        ):
...
#         self.Bind(wx.EVT_MENU, self.on_save_all_documents, id=ID_saveAllDocuments)
        ...
        if ((self._document_type in ['Chromatogram'] and self._indent == 2) or (
            self._document_type in [
                'Chromatograms (EIC)', 'Chromatograms (combined voltages, EIC)',
            ] and self._indent > 2
        )):
            try:
...
#         self.Bind(wx.EVT_MENU, self.presenter.onReExtractDTMS, id=ID_docTree_extractDTMS)
        ...
        # Change axis labels
        for xID in [
            ID_xlabel_2D_scans, ID_xlabel_2D_colVolt, ID_xlabel_2D_actVolt,
        ...
        for yID in [
            ID_ylabel_2D_bins, ID_ylabel_2D_ms, ID_ylabel_2D_ms_arrival,
        ...
        # 1D
        for yID in [
            ID_xlabel_1D_bins, ID_xlabel_1D_ms, ID_xlabel_1D_ms_arrival,
            ID_xlabel_1D_ccs, ID_xlabel_1D_restore,
        ]:
            self.Bind(wx.EVT_MENU, self.on_change_xy_values_and_labels, id=yID)
        # RT
        for yID in [
            ID_xlabel_RT_scans, ID_xlabel_RT_time_min, ID_xlabel_RT_retTime_min,
            ID_xlabel_RT_restore,
        ]:
            self.Bind(wx.EVT_MENU, self.on_change_xy_values_and_labels, id=yID)
        # DT/MS
        for yID in [
            ID_ylabel_DTMS_bins, ID_ylabel_DTMS_ms, ID_ylabel_DTMS_ms_arrival,
        ...
        if self._document_data.dataType == 'Type: Interactive':
            if self._document_type == 'Annotated data' and self._item_leaf != self._document_type:
                ...
                if (
                    'Multi-line: ' in self._item_leaf or 'V-bar: ' in self._item_leaf or
                    'H-bar: ' in self._item_leaf or 'Scatter: ' in self._item_leaf or
                        'Waterfall: ' in self._item_leaf or 'Line: ' in self._item_leaf
                ):
                    menu.AppendItem(
                        makeMenuItem(
                            parent=menu, id=ID_docTree_add_annotations,
        ...
        # tandem MS
            ...
            menu.AppendItem(
                makeMenuItem(
                    parent=menu, id=ID_docTree_add_mzIdentML,
                    text='Add identification information (.mzIdentML, .mzid, .mzid.gz)',
                    bitmap=None,
                ),
            )
#         elif itemType == "Tandem Mass Spectra" and self._indent == 3:
...
#                 menu.Append(ID_add2CCStable2DDocument, 'Add to CCS calibration window')
        ...
        # Input data
            ...
            # Only if clicked on a header
                ...
                menu.AppendItem(
                    makeMenuItem(
                        parent=menu, id=ID_docTree_addToTextTable,
                        text='Add to text file table',
                        bitmap=None,
                    ),
                )
...
#                                          bitmap=self.icons.iconsLib['file_png_16']))
        ...
        # Drift time (1D) (batch)
            ...
            menu.AppendItem(
                makeMenuItem(
                    parent=menu, id=ID_docTree_action_open_extractDTMS,
        ...
        if self._indent == 1:
            ...
            menu.AppendItem(
                makeMenuItem(
                    parent=menu, id=ID_docTree_duplicate_document,
        ...
        menu.AppendItem(
            makeMenuItem(
                parent=menu, id=ID_goToDirectory,
        ...
        menu.AppendItem(
            makeMenuItem(
                parent=menu, id=ID_saveAsInteractive,
        ...
        menu.AppendItem(
            makeMenuItem(
                parent=menu, id=ID_saveDocument,
        ...
        menu.AppendItem(
            makeMenuItem(
                parent=menu, id=ID_removeDocument,
    ...
    def on_change_xy_values_and_labels(self, evt):  # onUpdateXYaxisLabels
        ...
        if 'defaultY' not in data:
            data['defaultY'] = {
                'ylabels': data.get('ylabels', 'Intensity'),
                'yvals': data['yvals'],
            }
        ...
        if evtID in [
            ID_xlabel_2D_scans, ID_xlabel_2D_colVolt,
    ...
    def on_change_xy_axis(
        self, data, oldLabel, newLabel, charge=1,
        ...
        if oldLabel != newLabel:
            ...
            if (
                oldLabel == 'Drift time (bins)' and
                newLabel in ['Drift time (ms)', 'Arrival time (ms)']
            ):
                newVals = (data * pusherFreq) / 1000
                return newVals
            elif (
                oldLabel in ['Drift time (ms)', 'Arrival time (ms)'] and
                ...
                oldLabel == 'Collision Cross Section (Å²)' and
                newLabel in ['Drift time (ms)', 'Arrival time (ms)']
            ):
                self.presenter.onThreading(None, (msg, 4, 7), action='updateStatusbar')
                newVals = data
                ...
                oldLabel == 'Drift time (ms)' and newLabel == 'Arrival time (ms)' or
            ...
            # Convert CV --> LFE
            if (
                oldLabel in ['Collision Voltage (V)', 'Activation Energy (V)'] and
            ...
            # Convert Lab frame energy --> collision voltage
                ...
                newLabel in ['Collision Voltage (V)', 'Activation Energy (V)'] and
            ...
            # Convert LFE/CV --> scans
                ...
                newLabel == 'Scans' and
                oldLabel in [
                    'Lab Frame Energy (eV)', 'Collision Voltage (V)',
            ...
            # Convert Scans --> LFE/CV
                ...
                if defaults is None:
                    ...
                    if (
                        defaults['xlabels'] == 'Lab Frame Energy (eV)' or
            ...
            # Convert Scans -> Time
                ...
                newLabel in ['Retention time (min)', 'Time (min)'] and
                oldLabel == 'Scans'
            ):
                newVals = (data * scanTime) / 60
                return newVals
            elif (
                oldLabel in ['Retention time (min)', 'Time (min)'] and
                newLabel == 'Scans'
            ):
                newVals = (data / scanTime) * 60
                return newVals
    ...
    def onAddToTable(self, evt):
        ...
        if evtID == ID_docTree_addToMMLTable:
            ...
            for i, key in enumerate(data):
                ...
                textlist.Append([
                    minCE, maxCE, data[key]['charge'], color, data[key]['cmap'],
            ...
            for i, key in enumerate(data):
                ...
                document_label = '{}: {}'.format(document_title, key)
                textlist.Append([
                    '', minCE, maxCE, data[key].get('charge', ''), color, data[key]['cmap'],
                    data[key]['alpha'], data[key]['mask'], label, data[key]['zvals'].shape,
                    document_label,
                ])
            color = convertRGB1to255(color)
    ...
    def onShowMassSpectra(self, evt):
        ...
        self.panel_plot.on_plot_waterfall(
            xvals_list, yvals_list, None, colors=[],
    ...
    def onRenameItem(self, evt):
        ...
        if new_name == current_name:
            ...
            if self._indent == 1:
                ...
                try:
                    self.presenter.view.panelMML.onRenameItem(
                        current_name,
                        new_name,
                        item_type='document',
                    )
                    ...
                    pass
                try:
                    self.presenter.view.panelMultipleIons.onRenameItem(
                        current_name,
                        new_name,
                        item_type='document',
                    )
...
#                 except Exception: pass
    ...
    def onSaveUnidec(self, evt, data_type='all'):
        ...
        if ((self._document_type == 'Mass Spectrum' and self._item_leaf == 'UniDec') or
            ...
            self._document_type == 'Mass Spectrum' and self._item_branch == 'UniDec' and
            self._indent == 4
        ):
            unidec_engine_data = self._document_data.massSpectrum['unidec']
    ...
    def onShowUnidec(self, evt, plot_type='all'):
        ...
        # MS - all
        if self._document_type == 'Mass Spectrum' and self._item_leaf == 'UniDec':
            unidec_engine_data = self._document_data.massSpectrum['unidec']
        elif self._document_type == 'Mass Spectrum' and self._item_branch == 'UniDec':
            unidec_engine_data = self._document_data.massSpectrum['unidec']
            plot_type = self._item_leaf
        # Processed MS - all
        elif self._document_type == 'Mass Spectrum (processed)' and self._item_leaf == 'UniDec':
            unidec_engine_data = self._document_data.smoothMS['unidec']
        elif self._document_type == 'Mass Spectrum (processed)' and self._item_branch == 'UniDec':
            unidec_engine_data = self._document_data.smoothMS['unidec']
            plot_type = self._item_leaf
        # Multiple MS - all
        elif self._document_type == 'Mass Spectra' and self._item_leaf == 'UniDec':
            ...
            try:
                ...
                try:
                    self.panel_plot.on_plot_unidec_MW_add_markers(
                        unidec_engine_data['m/z with isolated species'],
        ...
        if plot_type in ['all', 'Charge information']:
            try:
                self.panel_plot.on_plot_unidec_ChargeDistribution(
                    unidec_engine_data['Charge information'][:, 0],
    ...
    def onShowPlot(self, evt, data_type=None, save_image=False):
        ...
        if self._document_type in 'Annotated data':
            ...
            try:
                ...
                return
            if plot_type in [
                'scatter', 'waterfall', 'line', 'multi-line', 'grid-scatter',
            ...
            if plot_type == 'scatter':
                self.panel_plot.on_plot_other_scatter(
                    xvals, yvals, zvals, xlabel, ylabel, colors, labels,
                    set_page=True, **kwargs
                )
            elif plot_type == 'waterfall':
                kwargs = {'labels': labels}
                self.panel_plot.on_plot_other_waterfall(
                    xvals, yvals, None, xlabel, ylabel, colors=colors,
                    set_page=True, **kwargs
                )
            elif plot_type == 'multi-line':
                self.panel_plot.on_plot_other_overlay(
                    xvals, yvals, xlabel, ylabel, colors=colors,
                    set_page=True, labels=labels,
                )
            elif plot_type == 'line':
                kwargs = {
                    'line_color': colors[0], 'shade_under_color': colors[0],
                    'plot_modifiers': data['plot_modifiers'],
                }
                ...
                self.panel_plot.on_plot_other_bars(
                    xvals, data['yvals_min'], data['yvals_max'],
                ...
                self.panel_plot.on_plot_matrix(
                    zvals=zvals, xylabels=yxlabels, cmap=cmap,
                    set_page=True,
                )
            else:
        ...
        # =======================================================================
            ...
            self._document_type in itemType for itemType in [
                'Mass Spectrum',
                'Mass Spectra',
            ...
            try:
                ...
                xlimits = [
                    self._document_data.parameters['startMS'],
                    self._document_data.parameters['endMS'],
                ]
            ...
            if self._document_data.dataType != 'Type: CALIBRANT':
                ...
                self.panel_plot.on_plot_MS_DT_calibration(
                    msX=msX, msY=msY, xlimits=xlimits,
        ...
        # =======================================================================
            ...
            if self._document_data.dataType != 'Type: CALIBRANT':
                ...
                self.panel_plot.on_plot_MS_DT_calibration(
                    dtX=dtX, dtY=dtY, xlabelDT=xlabel,
        ...
        # =======================================================================
            ...
            if out[0] == 'Grid (n x n)':
                ...
                if out[0] == 'Mask':
                    ...
                    self.panel_plot.on_plot_overlay_2D(
                        zvalsIon1=zvals1, cmapIon1=cmap1,
                ...
                self.panel_plot.on_plot_3D(
                    zvals=zvals, labelsX=xaxisLabels, labelsY=yaxisLabels,
            ...
            # Overlayed 1D data
                ...
                if out[0] == '1D':
                    ...
                    self.panel_plot.on_plot_overlay_DT(
                        xvals=xvals, yvals=yvals, xlabel=xlabels, colors=colors,
            ...
            if out[0] in ['Variance', 'Mean', 'Standard Deviation']:
                ...
                # Unpack data
                dataOut = self.presenter.get2DdataFromDictionary(
                    dictionary=data,
                    dataType='plot',
                    compact=True,
                )
    ...
    def onSaveDF(self, evt):
        ...
        if self._document_type == 'Mass Spectra' and self._item_leaf == 'Mass Spectra':
            ...
            try:
                ...
                print(
                    'Dataframe was saved in %s. It took: %s s.' %
    ...
    def onSaveData(self, data=None, labels=None, data_format='%.4f', **kwargs):
        ...
        dlg = wx.FileDialog(
            self.presenter.view, 'Please select a name for the file',
        ...
        if not kwargs.get('return_filename', False):
            if dlg.ShowModal() == wx.ID_OK:
                ...
                self.config.saveDelimiter = list(
                    wildcard_dict.keys(),
                )[
                    list(
    ...
    def onSaveCSV(self, evt):
        ...
        # Save MS - single
        if (
            self._document_type == 'Mass Spectrum' or self._document_type == 'Mass Spectrum (processed)' or
            self._document_type == 'Mass Spectra' and self. extractData != self._document_type
        ):
            # Default name
            defaultValue = 'MSout'
        ...
        # Save MS - multiple MassLynx fiels
            ...
            for key in self._document_data.multipleMassSpectrum:
                ...
                kwargs = {'default_name': defaultValue}
                self.onSaveData(
                    data=data, labels=[xlabel, 'Intensity'],
                    data_format='%.4f', **kwargs
                )
        ...
        # Save calibration parameters - single
            ...
            if evt.GetId() == ID_saveDataCSVDocument:
                saveFileName = self.presenter.getImageFilename(
                    prefix=True, csv=True,
        ...
        # Save RT - single
            ...
            if evt.GetId() == ID_saveDataCSVDocument:
                ...
                kwargs = {'default_name': defaultValue}
                self.onSaveData(
                    data=[rtX, rtY], labels=[xlabel, 'Intensity'],
                    data_format='%.4f', **kwargs
                )
        ...
        # Save 1D - single
            ...
            if evt.GetId() == ID_saveDataCSVDocument:
                ...
                kwargs = {'default_name': defaultValue}
                self.onSaveData(
                    data=[dtX, dtY], labels=[ylabel, 'Intensity'],
...
#             # Single mode
            ...
            if self._item_leaf == 'Drift time (1D, EIC, DT-IMS)':
                for key in self._document_data.IMS1DdriftTimes:
                    if self._document_data.dataType == 'Type: MANUAL':
                        ...
                        filename = ''.join([
                            self._document_data.path, '\\', saveFileName,
                            name[0], '_', name[1],
                            self.config.saveExtension,
                        ])
        ...
        # Save 1D/2D - batch + single
            ...
            # Save 1D/2D - single + batch
                ...
                # Save 1D/2D - batch
                if any(
                    self._item_leaf in extractData for extractData in [
                        'Drift time (2D, EIC)',
                        'Drift time (2D, combined voltages, EIC)',
                        'Drift time (2D, processed, EIC)',
                        'Input data', 'Statistical',
                    ...
                    # 1D - batch
                        ...
                        if not (
                            self._document_type == 'Input data'
                            or self._document_type == 'Statistical'
                        ):
                            for key in data:
                                stripped_key_name = key.replace(' ', '')
                ...
                # Save 1D/2D - single
                    ...
                    if out[0] == 'RMSD Matrix':
                        ...
                        filename = ''.join([
                            self._document_data.path, '\\', saveFileName,
                            self._item_leaf, self.config.saveExtension,
                        ])
                        zvals, xylabels, __ = self.presenter.get2DdataFromDictionary(
                            dictionary=data[self._item_leaf],
    ...
    #         self._document_data.moleculeDetails.get('molWeight', None)

    #         self.presenter.OnAddDataToCCSTable(filename=self._document_data.title,
    ...
    def onOpenDocInfo(self, evt):
        ...
        if self._indent == 2 and any(
            ...
            kwargs = {
                'currentTool': 'plot2D',
                'itemType': self._document_type,
                'extractData': None,
            }
            self.panelInfo = panelDocumentInfo(
                self, self.presenter, self.config, self.icons,
                document, **kwargs
            )
    ...
    def add_document(self, docData, expandAll=False, expandItem=None):
            ...
            for annotData in docData.multipleMassSpectrum:
                ...
                if 'unidec' in docData.multipleMassSpectrum[annotData]:
                    ...
                    for annotDataInner in docData.multipleMassSpectrum[annotData]['unidec']:
                        ...
                        self.SetPyData(
                            annotsItemInner,
                            docData.multipleMassSpectrum[annotData]['unidec'][annotDataInner],
                        )
                        self.SetItemImage(annotsItemInner, self.bulets_dict['mass_spec_on'], wx.TreeItemIcon_Normal)

                # add annotations
                if (
                    'annotations' in docData.multipleMassSpectrum[annotData] and
                    ...
                    for annotNameData in docData.multipleMassSpectrum[annotData]['annotations']:
                        ...
                        self.SetPyData(
                            annotsAnnotItem,
                            docData.multipleMassSpectrum[annotData]['annotations'][annotNameData],
                        )
        ...
        if len(docData.other_data) > 0:
            ...
            for annotData, __ in natsorted(list(docData.other_data.items())):
                ...
                # add annotations
                if (
                    'annotations' in docData.other_data[annotData] and
    ...
    def on_enable_document(
        self, getSelected=False, loadingData=False, highlightSelected=False,
    ...
    def enableCurrentDocument(
        self, getSelected=False, loadingData=False,
        ...
        if selected is not None:
            ...
            try:
                ...
                if text != 'Documents':
                    ...
                    self.view.SetTitle(
                        'ORIGAMI - %s - %s (%s)' %
                        (self.config.version, text, self._document_data.dataType),
                    )
            except Exception:
    ...
    def on_open_MGF_file(self, evt=None):
        dlg = wx.FileDialog(
            self.presenter.view, 'Open MGF file', wildcard='*.mgf; *.MGF',
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            title = 'Precursor: {:.4f} [{}]'.format(
                data['Scan 1']['scan_info']['precursor_mz'],
                data['Scan 1']['scan_info']['precursor_charge'],
            )
            self.panel_plot.on_plot_centroid_MS(
                data['Scan 1']['xvals'],
                data['Scan 1']['yvals'],
                title=title,
            )
    ...
    def on_open_mzML_file(self, evt=None):
        dlg = wx.FileDialog(
            self.presenter.view, 'Open mzML file', wildcard='*.mzML; *.MZML',
            style=wx.FD_DEFAULT_STYLE | wx.FD_CHANGE_DIR,
        )
        if dlg.ShowModal() == wx.ID_OK:
            tstart = time.time()
    ...
    def on_add_mzID_file(self, evt):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            tandem_spectra = reader.match_identification_with_peaklist(
                peaklist=deepcopy(document.tandem_spectra),
    ...
    def on_open_thermo_file(self, evt):
        dlg = wx.FileDialog(
            self.presenter.view, 'Open Thermo file', wildcard='*.raw; *.RAW',
            style=wx.FD_DEFAULT_STYLE | wx.FD_CHANGE_DIR,
        )
        if dlg.ShowModal() == wx.ID_OK:
            tstart = time.time()
        ...
        if item is not False and not set_data_only:
            ...
            # add extracted spectrum
                ...
                self.add_one_to_group(
                    item, document.multipleMassSpectrum[item_name], item_name,
                    image=data_type,
                )
            # add extracted spectrum
            elif data_type == 'extracted.chromatogram':
            ...
            # add heatmap-raw data
                ...
                self.add_one_to_group(
                    item, document.IMS2Dions[item_name], item_name,
                    image=data_type,
                )
            # add CIU-style data
            elif data_type == 'ion.heatmap.combined':
