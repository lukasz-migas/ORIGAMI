...
class TestWatersRawReader:
    ...
    def test_check_fcn(self, get_waters_im_small):
        reader = WatersRawReader(get_waters_im_small)
        with pytest.raises(ValueError):
            reader.check_fcn(reader.n_functions + 1)

    def test_get_tic(self, get_waters_im_small):
        reader = WatersRawReader(get_waters_im_small)
        x, y = reader.get_tic()
        assert len(x) == len(y)

    def test_get_bpi(self, get_waters_im_small):
        reader = WatersRawReader(get_waters_im_small)
        x, y = reader.get_bpi()
        assert len(x) == len(y)
    ...
    def test_get_chromatogram(self, get_waters_im_small, mz_value, count, tolerance):
        reader = WatersRawReader(get_waters_im_small)
    ...
    def test_get_chromatogram_wrong_tolerance(self, get_waters_im_small, tolerance):
        reader = WatersRawReader(get_waters_im_small)
        with pytest.raises(ValueError):
    ...
    @pytest.mark.parametrize("mz_value, count", ([500, 1], [[500, 600], 2], [(500, 600), 2]))
    @pytest.mark.parametrize("tolerance", (0.1, 0.5))
    def test_get_mobilogram(self, get_waters_im_small, mz_value, count, tolerance):
        reader = WatersRawReader(get_waters_im_small)
    ...
    @pytest.mark.parametrize("tolerance", (None, -1, 0))
    def test_get_mobilogram_wrong_tolerance(self, get_waters_im_small, tolerance):
        reader = WatersRawReader(get_waters_im_small)
        with pytest.raises(ValueError):
    ...
    def test_get_spectrum(self, get_waters_im_small, start_scan, end_scan):
        reader = WatersRawReader(get_waters_im_small)
        x, y = reader.get_spectrum(start_scan, end_scan)
        assert len(x) == len(y)
        assert y.dtype == np.float32

    def test__get_spectrum(self, get_waters_im_small):
        reader = WatersRawReader(get_waters_im_small)
        x, y = reader._get_spectrum(0, 1)
        assert len(x) == len(y)

    def test_get_average_spectrum(self, get_waters_im_small):
        reader = WatersRawReader(get_waters_im_small)
    ...
    def test_get_drift_spectrum(self, get_waters_im_small, start_scan, end_scan, start_drift, end_drift):
        reader = WatersRawReader(get_waters_im_small)
