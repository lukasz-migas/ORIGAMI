...
class PanelImagingImportDataset(MiniFrame):
    ...
    _peaklist_peaklist = {
        ...
        1: {"name": "index", "tag": "index", "type": "int", "show": True, "width": 100},
        2: {"name": "filename", "tag": "filename", "type": "str", "show": True, "width": 300},
        3: {"name": "m/z range", "tag": "mz_range", "type": "str", "show": True, "width": 100},
        4: {"name": "# scans", "tag": "n_scans", "type": "str", "show": True, "width": 80},
        5: {"name": "scan range", "tag": "scan_range", "type": "str", "show": True, "width": 100},
        6: {"name": "IM", "tag": "ion_mobility", "type": "str", "show": True, "width": 50},
    }
    ...
    def on_update_info(self):
        ...
        if self.config.ms_process_crop:
            info += f"Crop\n"
            info += f"  m/z range: {self.config.ms_crop_min:.2f} - {self.config.ms_crop_max:.2f}\n"
    ...
    def on_update_import_info(self):
        ...
        if len(mz_range) > 1 or len(im_on) > 1:
    ...
    def on_select_files(self, evt):
        ...
        # get paths
        #         filelist = self.data_handling.on_open_multiple_LESA_MassLynx_raw_fcn()
        filelist = [
            "D:\\Data\\ORIGAMI\\LESA MSI for ORIGAMI\\2019_04_26_013_MOUSE_BRAIN_001.raw",
            "D:\\Data\\ORIGAMI\\LESA MSI for ORIGAMI\\2019_04_26_014_MOUSE_BRAIN_002.raw",
            "D:\\Data\\ORIGAMI\\LESA MSI for ORIGAMI\\2019_04_26_015_MOUSE_BRAIN_003.raw",
        ]
        # check paths
        ...
        for path in filelist:
            if self.check_present(path):
                logger.warning(f"File {path} already in the list")
                continue
            ...
            # add to table
            self.peaklist.Append(["", idx, path, f"{mz_range[0]}-{mz_range[1]}", f"{n_scans}", "", str(is_im)])

        self.on_update_import_info()

    def check_present(self, path):
        n_rows = self.peaklist.GetItemCount()

        for item_id in range(n_rows):
            information = self.on_get_item_information(item_id)
            if information["filename"] == path:
                return True
        return False
    ...
    def make_settings_panel(self, split_panel):
        ...
        msg = "Please select files and edit pre-processing steps in order to continue."
        ...
        msg = ""
        self.import_label = wx.StaticText(panel, wx.ID_ANY, msg)
        ...
        n = 0
        btn_grid.Add(self.processing_btn, (n, 0), flag=wx.ALIGN_CENTER)
        ...
        n += 3
        grid_2.Add(self.import_btn, (n, 0), flag=wx.ALIGN_CENTER)
        ...
        main_sizer.Add(horizontal_line_1, 0, wx.EXPAND, 10)
        main_sizer.Add(self.peaklist, 1, wx.EXPAND, 10)
        main_sizer.Add(grid_2, 0, wx.ALL | wx.EXPAND, 5)
