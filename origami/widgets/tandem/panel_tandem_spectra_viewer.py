...
# Local imports
from origami.ids import ID_tandemPanel_showPTMs
from origami.ids import ID_tandemPanel_otherSettings
from origami.ids import ID_tandemPanel_sort_peptide_by_AZ
from origami.ids import ID_tandemPanel_showUnidentifiedScans
...
class PanelTandemSpectraViewer(wx.MiniFrame):
    ...
    def _generate_peaklist_config(self):
        ...
        self._mod_columns = {"check": 0, "peptide": 1, "name": 2, "delta": 3, "residues": 4, "location": 5}
    ...
    def on_tool_menu(self, evt):
        ...
        self.menu_tool_sort_peptide_az = make_menu_item(
            parent=menu, evt_id=ID_tandemPanel_sort_peptide_by_AZ, text="Sort by A-Z", bitmap=None
        )
        menu.AppendItem(self.menu_tool_sort_peptide_az)
        self.menu_tool_sort_peptide_length = make_menu_item(
            parent=menu, evt_id=ID_tandemPanel_sort_peptide_by_length, text="Sort by size", bitmap=None
        )
    ...
    def on_sort(self, evt):
        ...
        if evt.GetId() == ID_tandemPanel_sort_peptide_by_AZ:
            ...
            self.sort_by_column_peaklist(column=self.__gui__["peaklist_all_column"], sort_type="length")
    ...
    def onRightClickMenu_peaklist(self, evt):
        ...
        menu.AppendItem(
            make_menu_item(
                parent=menu, evt_id=ID_tandemPanel_peaklist_show_selected, text="Hide unselected items", bitmap=None
            )
        )
        self.PopupMenu(menu)
        menu.Destroy()
    ...
    def on_check_for_duplicates_peaklist_selected(self, table_row):
        ...
        for i in range(rows):
            ...
            if scan_id == scan_id_table and scan_id_id == scan_id_id_table and scan_title == scan_title_table:
                return True
    ...
    def on_delete_all_annotations(self, evt):
        ...
        dlg = DialogBox(
            "Are you sure?",
            "Are you sure you want to remove all annotations from the document? This action is irreversible.",
            "Question",
        )
        if dlg == wx.ID_NO:
    ...
    def make_peptide_checkbox(self, panel):
        ...
        self.tolerance_choice = wx.Choice(panel, -1, choices=self.config.fragments_units_choices, size=(-1, -1))
        ...
        self.max_labels_value = wx.SpinCtrlDouble(
            panel,
            -1,
            value=str(self.config.fragments_max_matches),
            min=1,
            max=5,
            initial=self.config.fragments_max_matches,
            inc=1,
            size=(90, -1),
        )
    ...
    def make_peptide_list(self, panel):
        ...
        self.fraglist = ListCtrl(
            panel, style=wx.LC_REPORT | wx.LC_VRULES, column_info=self._fragment_peaklist, use_simple_sorter=True
        )
    ...
    def make_modification_list(self, panel):
        ...
        self.modlist = ListCtrl(
            panel, style=wx.LC_REPORT | wx.LC_VRULES, column_info=self._modification_peaklist, use_simple_sorter=True
        )
    ...
    def make_peaklist_selected_list(self, panel):
        ...
        for col in range(len(self._selected_peaklist_peaklist)):
            ...
            self.peaklist_selected.SetFont(wx.Font(8, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
    ...
    def make_panel(self):
        ...
        self.peaklist_selected = wx.Panel(
            self.peaklistBook, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.TAB_TRAVERSAL
        )
        ...
        self.annotation_peaklist = wx.Panel(
            self.annotationBook, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.TAB_TRAVERSAL
        )
        ...
        self.show_count_value = wx.SpinCtrlDouble(
            panel,
            -1,
            value=str(self.config.msms_load_n_scans),
            min=10,
            max=999999,
            inc=500,
            initial=self.config.msms_load_n_scans,
            size=(90, -1),
        )
    ...
    def onActionTool(self, evt):
        ...
        menu.AppendItem(
            make_menu_item(
                parent=menu,
                evt_id=ID_tandemPanel_otherSettings,
    ...
    def get_item_info_fragments(self, itemID, return_list=False):
        # get item information
        information = {
            "measured_mz": str2num(self.fraglist.GetItem(itemID, self._frag_columns["measured m/z"]).GetText()),
    ...
    def on_apply(self, evt):
        ...
        _fragments = {
            "M-ALL": self.peptide_M_all.GetValue(),
            "M": self.peptide_M.GetValue(),
            "M-nH2O": self.peptide_M_nH2O.GetValue(),
            "M-nNH3": self.peptide_M_nNH3.GetValue(),
            "a-ALL": self.peptide_a_all.GetValue(),
            "a": self.peptide_a.GetValue(),
            "a-nH2O": self.peptide_a_nH2O.GetValue(),
            "a-nNH3": self.peptide_a_nNH3.GetValue(),
            "b-ALL": self.peptide_b_all.GetValue(),
            "b": self.peptide_b.GetValue(),
            "b-nH2O": self.peptide_b_nH2O.GetValue(),
            "b-nNH3": self.peptide_b_nNH3.GetValue(),
            "c-ALL": self.peptide_c_all.GetValue(),
            "c": self.peptide_c.GetValue(),
            "c-nH2O": self.peptide_c_nH2O.GetValue(),
            "c-nNH3": self.peptide_c_nNH3.GetValue(),
            "c-dot": self.peptide_c_dot.GetValue(),
            "c+1/2": self.peptide_c_add_1_2.GetValue(),
            "x-ALL": self.peptide_x_all.GetValue(),
            "x": self.peptide_x.GetValue(),
            "x-nH2O": self.peptide_x_nH2O.GetValue(),
            "x-nNH3": self.peptide_x_nNH3.GetValue(),
            "y-ALL": self.peptide_y_all.GetValue(),
            "y": self.peptide_y.GetValue(),
            "y-nH2O": self.peptide_y_nH2O.GetValue(),
            "y-nNH3": self.peptide_y_nNH3.GetValue(),
            "z-ALL": self.peptide_z_all.GetValue(),
            "z": self.peptide_z.GetValue(),
            "z-nH2O": self.peptide_z_nH2O.GetValue(),
            "z-nNH3": self.peptide_z_nNH3.GetValue(),
            "z-dot": self.peptide_z_dot.GetValue(),
            "z+1/2": self.peptide_z_add_1_2.GetValue(),
        }
    ...
    def onUpdateGUI(self, evt):
        ...
        if self.peptide_c_all.GetValue():
            enableList.extend(
                [self.peptide_c, self.peptide_c_nH2O, self.peptide_c_nNH3, self.peptide_c_dot, self.peptide_c_add_1_2]
            )
        ...
        if self.peptide_z_all.GetValue():
            enableList.extend(
                [self.peptide_z, self.peptide_z_nH2O, self.peptide_z_nNH3, self.peptide_z_dot, self.peptide_z_add_1_2]
            )
    ...
    def on_zoom_on_spectrum(self, evt):
        ...
        intensity = str2num(itemInfo["intensity"]) * 2.0
    ...
    def on_populate_modification_table(self, peptide, mod_dict, n_id):
        ...
        for modification in natsorted(modifications):
            ...
            self.modlist.Append(["", str(peptide), str(name), str(mass_delta), str(residues), str(location)])
