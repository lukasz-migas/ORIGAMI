...
class mzFile(mzAPImzFile):
    """
    mzAPI access to mzFile files. Loads everything from the XML into a sqlite3
    ...
    The intermediary sqlite3 files can be saved for later use by calling
    ...
    def save_cache(self, save_file):
        ...
        shutil.move(self.cachename, save_file)
    ...
    def scan_for_time(self, time, tolerance=0.0001):
        ...
        self.cursor.execute(
            "SELECT ind, rt FROM spectra WHERE rt >= {} AND rt <= {}".format(
