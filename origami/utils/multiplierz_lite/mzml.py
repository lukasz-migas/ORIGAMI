...
def cvp(parent, cvRef="MS", unitCvRef="MS", **parameters):
    return xml.SubElement(parent, ns("cvParam"), cvRef=cvRef, unitCvRef=unitCvRef, **parameters)


def uncvp(el):
    assert el.tag == ns("cvParam")
    return {"name": el.get("name"), "units": el.get("unitName"), "value": el.get("value")}
...
def makeSpectrumXML(specData):
    ...
    scanlistdataEl = xml.SubElement(specEl, ns("scanList"), count=1)
    scanEl = xml.SubElement(
        scanlistdataEl, ns("scan"), sourceFileRef=specData["Source"], spectrumRef=specData["Spectrum Description"]
    )
    ...
    if specData["MS Level"] != "MS1":
        ...
        precEl = xml.SubElement(
            precList, ns("precursor"), sourceFileRef=specData["Source"], spectrumRef=specData["previous MS1"]
        )
        ...
        if specData["charge"]:
            cvp(selIonEl, accession="MS:1000041", name="charge state", value=specData["charge"])
    ...
    mzDataArray = xml.SubElement(
        binaryDataArrayList, ns("binaryDataArray"), arrayLength=len(specData["spectrum"]), encodedLength=len(encodedMZs)
    )
