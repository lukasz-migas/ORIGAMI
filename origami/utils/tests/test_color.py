...
class TestColor(object):
    ...
    def test_convertRGB1to255():
        ...
        return_value = convertRGB1to255([1.0, 1.0, 1.0], decimals=1, as_integer=False)

        assert expected_value == return_value

        # check integer
        expected_value = [255, 255, 255]
        return_value = convertRGB1to255([1.0, 1.0, 1.0], decimals=1, as_integer=True)
    ...
    def test_convertRGB255to1():
        ...
        return_value = convertRGB255to1([255.0, 255.0, 255.0], decimals=1)
    ...
    def test_determineFontColor():
        ...
        return_value = determineFontColor([0.0, 0.0, 0.0], return_rgb=True)

        assert expected_value == return_value

        # check font color and return string
        expected_value = (255, 255, 255)  # white
        return_value = determineFontColor([0.0, 0.0, 0.0], return_rgb=True, convert1to255=True)

        assert expected_value == return_value

        # check font color and return white
        expected_value = "white"  # white
        return_value = determineFontColor([0.0, 0.0, 0.0], return_rgb=False)
